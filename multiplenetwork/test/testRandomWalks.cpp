...
void testRandomWalks() {
	...
	//log("Testing ...",false);
	NodeSharedPtr n0 = mnet3->get_node("U0",mnet3->get_layer("l1"));
	NodeSharedPtr n1 = mnet3->get_node("U1",mnet3->get_layer("l1"));
	Path p(mnet3,n0);
	...
	while (stop_condition--) {
		 	if (random_utils::test(.8)) {
				ObjectStore<NodeSharedPtr> neigh = mnet3->neighbors(rw.end(),OUT);
				...
				log("Actor change");
				ObjectStore<NodeSharedPtr> nodes = mnet3->get_nodes(rw.end()->actor);
				if (nodes.size()==0)
					break;
